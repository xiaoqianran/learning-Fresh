...
export const TRACK_META: Record<Lesson["track"], { order: number; label: string; blurb: string }> =
  {
    基础: { order: 1, label: "① 入门", blurb: "路由 · Islands · Signals" },
    进阶: { order: 2, label: "② 组件进阶", blurb: "布局 · 中间件 · Handler" },
    全栈准备: { order: 3, label: "③ 全栈基础", blurb: "API · 鉴权 · 表单" },
    全栈实训: { order: 4, label: "④ 项目实训", blurb: "REST · KV · Deploy · 毕业" },
    工程化: { order: 5, label: "⑤ 工程化", blurb: "Deno · 测试 · CI" },
    进阶模式: { order: 6, label: "⑥ 模式与面试", blurb: "切岛 · 性能 · 串讲" },
    /** 可选加深：与 LESSONS 数组一致，放在主路径之后，避免「继续学习」与路径卡错位 */
    官网对齐: { order: 7, label: "⑦ 官网补全", blurb: "对照官网 · 可选加深" },
  };
...
export const NAV_TOOLS: NavItem[] = [
  ...
  { to: "/playground", label: "代码实验室", hint: "routes / islands 示例", icon: Code2 },
